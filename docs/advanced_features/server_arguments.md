...
| `--hicache-io-backend` | The IO backend for KV cache transfer between CPU and GPU | `kernel` | `direct`, `kernel`, `kernel_ascend`, `hybrid` |
| `--enable-hybrid-balanced-batch` | Enable Balanced Batch Formation for hybrid HiCache prefill batches. Requires `--hicache-io-backend=hybrid`. | `False` | bool flag (set to enable) |
| `--enable-hybrid-bubble-filling` | Enable decode bubble filling while hybrid HiCache prefill preload is still in flight. Requires `--hicache-io-backend=hybrid`. | `False` | bool flag (set to enable) |
